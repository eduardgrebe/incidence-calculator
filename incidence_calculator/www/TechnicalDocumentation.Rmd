...
* More specifically, optimisation of simple tests involves adjustment of some recent/non-recent threshold, which leads to a trade-off between the Mean Duration of Recent Infection (MDRI) and False Recent Rate (FRR) (see glossary). A larger MDRI means a greater (more precisely estimable) number of recent cases in a cross sectional survey, but this eventually and inevitably comes at the cost of a non trivial FRR, the accounting of which leads to the growth of an additional high-variance term in the estimator.
* It bears pointing out that there is no *gold standard* for a test for recent infection, in anything resembling the meaning of the term that is appropriate for a discussion of usual clinical diagnostics. In particular, there is no natural, or selectable time scale for ‘recent infection’ which a recency test is then aimed at detecting. The case definition for recent infection is, then, essentially defined arbitrarily, by the biomarker, which becomes the gold standard, if one must have one. Of course it is statistically informative (for incidence estimation) only if the case definition is something usually exhibited by newly infected individuals, and very rarely exhibited by people infected for substantial periods like several years.
...
The package inctools, and discussion of applications built on the inctools infrastructure, implies use of some common or specialised (to this sub-field) terms which it may be prudent to define here. These are rendered as conceptual rather than technical definitions.
...
On account of the fact that it is always possible for a recency test to classify some individuals as recently infected at long times post infection, the use of a time cut-off T has been introduced to assist in the mathematical housekeeping.  The details of how this works are well beyond the scope of this user guide, but if using a recent infection test based on a reasonable body of literature, there is little risk in adopting previously proposed cut off times.
...
Within *inctools*, there is provision for users to provide scale factors adjusting the uncertainty in estimates of
